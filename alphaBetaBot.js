class AlphaBetaPlayer extends Player {
    ...
    findBestMove() {
        ...
        for (let i = 0; i < this.gameBoard.length; i++) {
            if (this.gameBoard[i] === '') {
                ...
                let moveVal = this.alphaBeta(this.gameBoard, 0, false, alpha, beta);
                this.gameBoard[i] = '';
    ...
    alphaBeta(board, depth, isMaximizing, alpha, beta) {
        ...
        if (isMaximizing) {
            ...
            for (let i = 0; i < board.length; i++) {
                if (board[i] === '') {
                    board[i] = this.symbol;
                    best = Math.max(best, this.alphaBeta(board, depth + 1, false, alpha, beta));
                    board[i] = '';
            ...
            for (let i = 0; i < board.length; i++) {
                if (board[i] === '') {
                    ...
                    best = Math.min(best, this.alphaBeta(board, depth + 1, true, alpha, beta));
                    board[i] = '';
